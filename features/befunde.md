...
| BF-99 | 05 | Der Schriftzug der beiden Wort-Bildmarken liegt als `<text>` vor, nicht als Pfad. Ohne die Schrift auf dem Zielsystem ersetzt der Betrachter sie — bei einer Wortmarke ist die Schrift die Marke. Die Bildmarke selbst ist sauber vektorisiert. Kein Codeanteil: einmal in Illustrator oder Affinity outlinen, dann `make press-kit` erneut | mittel | `public/presse/endlech-wortbildmarke.svg`, `…-invers.svg` | offen — ⚠ **am 2026-09-12 geschärft: Die Ursache ist nicht der fehlende Pfad, sondern die fehlende Markenschrift.** Die `<text>`-Elemente führen `font-family` mit „ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif“ — also die **Systemschrift** des jeweiligen Geräts, keine festgelegte Schrift. ⚠ **Wer die Datei jetzt öffnet und „Text in Pfade konvertieren“ klickt, friert die Schrift ein, die sein Rechner gerade zeigt** (auf einem Mac SF Pro, auf Windows Segoe UI) — und legt damit die Wortmarke unbeabsichtigt fest. Zuerst gehört entschieden, welche Schrift die Marke **ist**; das ist eine Betreiber- und keine Werkzeugfrage. Danach outlinen und `make press-kit` erneut. |
